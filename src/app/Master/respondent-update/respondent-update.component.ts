...
@Component({
  selector: 'app-respondent-update',
  templateUrl: './respondent-update.component.html',
  styleUrls: ['./respondent-update.component.css']
})
export class RespondentUpdateComponent implements OnInit {
  ...
  onforminit(userData: any) {
    this.saveForm = this.fb.group({
      res_employee: [userData?.res_employee],
      res_name: [userData.res_name,Validators.required],
      res_designation: [userData.res_designation,],
      res_mobile: [userData.res_mobile,[ Validators.pattern('^[0-9][0-9]{9}$')]],
      res_email: [userData.res_email,[Validators.pattern('^[a-zA-Z0-9+_.-]+@[a-zA-Z0-9.-]+$'),]],
